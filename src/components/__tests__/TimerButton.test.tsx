...
describe('TimerButton', () => {
  // Тест 1: Проверка корректного отображения кнопки старта
  test('renders start button in initial state', () => {
  ...
  // Тест 2: Проверка корректного отображения кнопки паузы при запущенном таймере
  ...
  test('buttons have correct accessibility classes', () => {
    ...
    // Проверка наличия CSS классов (transition-colors используется в Button компоненте)
    expect(pauseButton).toHaveClass('transition-colors');
    expect(stopButton).toHaveClass('transition-colors');
  });
  ...
  test('applies different styles based on timer state', () => {
    ...
    // Проверяем что кнопка имеет базовые стили Button компонента
  ...
  // Тест 11: Проверка keyboard accessibility
  ...
  // Тест 12: Проверка button rendering with long text
  ...
  // Тест 13: Проверка that stop button doesn't appear in initial state
  ...
  // Тест 14: Проверка the container's flex layout
