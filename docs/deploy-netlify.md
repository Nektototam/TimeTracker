...
1. Создайте файл `netlify.toml` в корне проекта со следующим содержимым:
...
1. Убедитесь, что в файле `.env.example` указаны все необходимые переменные окружения:
...
1. Убедитесь, что `.env.local` добавлен в `.gitignore` для исключения секретных данных из репозитория.
